...
public class UISlider : IgnoreTimeScale
{
	...
	private void Init()
	{
		...
		{
			UnityEngine.Debug.LogWarning("UISlider expected to find a foreground object or a box collider to work with", this);
		}
	}
		...
		if (force || sliderValue != sliderValue2)
		{
			...
			if (this.thumb != null)
			{
				...
				if (this.mFGFilled != null && this.mFGFilled.type == UISprite.Type.Filled)
				{
					...
					{
						UnityEngine.Debug.LogWarning("Slider thumb is only supported with Horizontal or Vertical fill direction", this);
					}
				}
