	...
	public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (this.effectStyle != UILabel.Effect.None)
		{
			...
			if (this.effectStyle == UILabel.Effect.Outline)
			{
				...
				size = verts.size;
				this.ApplyShadow(verts, uvs, cols, start, size, -num2, -num3);
				if (this.mEffectDistance.x > 1f && this.mEffectDistance.y > 1f)
				{
					start = size;
					size = verts.size;
					this.ApplyShadow(verts, uvs, cols, start, size, 0f, num3);
					start = size;
					size = verts.size;
					this.ApplyShadow(verts, uvs, cols, start, size, 0f, -num3);
					start = size;
					size = verts.size;
					this.ApplyShadow(verts, uvs, cols, start, size, num2, 0f);
					start = size;
					size = verts.size;
					this.ApplyShadow(verts, uvs, cols, start, size, -num2, 0f);
				}
