...
internal class FortressWarStandartTerrain : ITerrainGenerationMethod
{
	...
	private void LandPass(WorldData worldData, Chunk bottomChunk, int noiseBlockOffset)
	{
		...
		for (int i = 0; i < worldData.ChunkBlockWidth; i++)
		{
			...
			{
				int num4 = num2 + j;
				float lowerGroundHeight = FortressWarStandartTerrain.GetLowerGroundHeight(num3, num4, i, j, worldData.DepthInBlocks, noiseBlockOffset);
				int upperGroundHeight = FortressWarStandartTerrain.GetUpperGroundHeight(worldData, num3, num4, lowerGroundHeight, noiseBlockOffset);
