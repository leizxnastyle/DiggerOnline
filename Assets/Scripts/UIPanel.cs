	...
	public UIPanel.DebugInfo debugInfo
	{
		...
		set
		{
			if (this.mDebugInfo != value)
			{
				...
				HideFlags hideFlags = (this.mDebugInfo != UIPanel.DebugInfo.Geometry) ? HideFlags.HideAndDontSave : (HideFlags.DontSaveInEditor | HideFlags.NotEditable | HideFlags.DontSaveInBuild | HideFlags.DontUnloadUnusedAsset);
				int i = 0;
	...
	private void UpdateTransformMatrix()
	{
		if (this.mUpdateTime == 0f || this.mMatrixTime != this.mUpdateTime)
		{
			...
			if (this.mClipping != UIDrawCall.Clipping.None)
			{
				...
				if (a.x == 0f)
				{
					a.x = (float)((!(this.mCam == null)) ? this.mCam.pixelWidth : Screen.width);
				}
				if (a.y == 0f)
				{
					a.y = (float)((!(this.mCam == null)) ? this.mCam.pixelHeight : Screen.height);
				}
				a *= 0.5f;
