...
public class UIForwardEvents : MonoBehaviour
{
	...
	{
		if (this.onInputChanged && this.target != null)
		{
			this.target.SendMessage("OnInputChanged", base.gameObject, SendMessageOptions.DontRequireReceiver);
		}
	}

	private void OnTooltip(bool show)
	{
		if (this.onTooltip && this.target != null)
		{
			if (show)
			{
				this.target.SendMessage("OnTooltipShow", base.gameObject, SendMessageOptions.DontRequireReceiver);
			}
			else
			{
				this.target.SendMessage("OnTooltipHide", base.gameObject, SendMessageOptions.DontRequireReceiver);
			}
		}
	}
