...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	private void Start()
	{
		...
		{
			this.verticalScrollBarValue = this.verticalScrollBar.sliderValue;
		}
	}
	...
	{
		Bounds bounds = this.bounds;
		if (bounds.size.y < this.mPanel.clipRange.w)
		{
			bounds.min = new Vector3(bounds.min.x, bounds.min.y - (this.mPanel.clipRange.w - bounds.size.y), bounds.min.z);
		}
	...
	public void UpdateScrollbars(bool recalculateBounds)
	{
		...
		if (this.horizontalScrollBar != null || this.verticalScrollBar != null)
		{
			...
			if (this.verticalScrollBar != null && a2.y > a.y)
			{
				...
				this.mIgnoreCallbacks = true;
				this.verticalScrollBar.sliderValue = 1f - ((num10 <= 0.001f) ? 0f : (1f - num7 / num10));
				this.verticalScrollBarValue = this.verticalScrollBar.sliderValue;
				this.mIgnoreCallbacks = false;
			}
		}
	...
	private void OnVerticalBar(float val)
	{
		if (!this.mIgnoreCallbacks)
		{
			float x = (!(this.horizontalScrollBar != null)) ? 0f : this.horizontalScrollBar.scrollValue;
			float y = (!(this.verticalScrollBar != null)) ? 0f : (1f - val);
	...
	{
		if (this.verticalScrollBar != null && this.verticalScrollBar.gameObject.activeInHierarchy && this.verticalScrollBar.sliderValue != this.verticalScrollBarValue)
		{
			this.verticalScrollBarValue = this.verticalScrollBar.sliderValue;
			this.OnVerticalBar(this.verticalScrollBarValue);
		}
		...
		if (this.showScrollBars != UIDraggablePanel.ShowCondition.Always)
		{
			...
			{
				flag = this.shouldMoveHorizontally;
			}
	...
	public float verticalScrollBarValue;
