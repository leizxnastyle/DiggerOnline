...
public class AutumnTreeDecorator : IDecoration
{
	public AutumnTreeDecorator(WorldData worldData)
	{
	...
	private void CreatePlain(int blockX, int blockY, int blockZ, int radius)
	{
		for (int i = blockX - radius; i <= blockX + radius; i++)
		{
			...
			{
				this.m_WorldData.SetBlockType(i, j, blockZ, BlockType.Autumn3);
			}
		}
	}
	...
	{
		this.m_WorldData.SetBlockType(blockX, blockY, z, BlockType.Autumn2);
	}
