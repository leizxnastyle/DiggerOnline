		...
		public override double GetValue(double x, double y, double z)
		{
			...
			for (int i = num3 - 2; i <= num3 + 2; i++)
			{
				for (int j = num2 - 2; j <= num2 + 2; j++)
				{
					...
					{
						double num8 = (double)k + LibNoizeUtils.ValueNoise3D(k, j, i, this.m_seed);
						double num9 = (double)j + LibNoizeUtils.ValueNoise3D(k, j, i, this.m_seed + 1);
						double num10 = (double)i + LibNoizeUtils.ValueNoise3D(k, j, i, this.m_seed + 2);
						double num11 = num8 - x;
						double num12 = num9 - y;
			...
			return num18 + this.m_displacement * LibNoizeUtils.ValueNoise3D((int)Math.Floor(num5), (int)Math.Floor(num6), (int)Math.Floor(num7), 0);
		}
