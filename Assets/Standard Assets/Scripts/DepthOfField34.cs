...
public class DepthOfField34 : PostEffectsBase
{
	...
	{
		this.CreateMaterials();
		if (this.smoothness < (float)0)
		{
			this.smoothness = (float)0;
		}
		bool flag;
		if (flag = this.bokeh)
		{
		...
		if (this.bokeh && (this.bokehDestination & BokehDestination.Background) != (BokehDestination)0)
		{
			...
			Graphics.Blit(this.mediumRezWorkTexture, this.lowRezWorkTexture, this.dofMaterial, 10);
	...
	public virtual void AllocateTextures(bool blurForeground, RenderTexture source, int divider, int lowTexDivider)
	{
		...
		if (this.bokeh)
		{
			this.bokehSource = RenderTexture.GetTemporary(source.width / (lowTexDivider * this.bokehDownsample), source.height / (lowTexDivider * this.bokehDownsample), 0);
			this.bokehSource2 = RenderTexture.GetTemporary(source.width / (lowTexDivider * this.bokehDownsample), source.height / (lowTexDivider * this.bokehDownsample), 0);
